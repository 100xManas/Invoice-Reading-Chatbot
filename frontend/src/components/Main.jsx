  ...
  const handleFileChange = async (e) => {
    ...
    if (selectedFile) {
      ...
      try {
        ...
        if (data.success) {
          setDocumentId(data.invoiceData.invoice_number);

          setMessages([
            {
              role: "assistant",
              text: "I've processed your invoice! You can now ask me questions about it.",
            },
          ]);
        } else {
          setMessages([
      ...
      {activeTab === "upload" && (
        <section className="flex flex-col items-center justify-center mb-8">
          ...
          <div className="mt-8 text-center text-gray-400 max-w-lg">
            ...
            <p className="text-sm">
              After uploading, switch to the Chat tab to ask questions about
              your invoice
            </p>
          </div>
        </section>
        ...
        <section className="mx-auto w-full max-w-3xl bg-white/5 backdrop-blur-md border border-zinc-700 rounded-xl shadow-lg p-4 md:p-6 transition-all hover:border-red-400/30">
          ...
          {!file && (
            <p className="text-sm text-red-400 mt-2">
              Please upload an invoice first to start chatting
            </p>
          )}
        </section>
      )}
