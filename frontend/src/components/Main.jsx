    ...
    <main className="flex-1 w-full px-4 py-8 md:px-8 lg:px-16">
      ...
      {activeTab === "upload" && (
        <section className="flex flex-col items-center justify-center mb-8">
          <h2 className="text-2xl md:text-3xl font-bold text-red-500 mb-6 text-center">Upload Your Invoice</h2>
          ...
          <div className="mt-8 text-center text-gray-400 max-w-lg">
            <p className="mb-2">Supported formats: PDF, JPG, PNG</p>
            <p className="text-sm">After uploading, switch to the Chat tab to ask questions about your invoice</p>
          </div>
        </section>
      )}
      ...
      {activeTab === "chat" && (
        <section className="mx-auto w-full max-w-3xl bg-white/5 backdrop-blur-md border border-zinc-700 rounded-xl shadow-lg p-4 md:p-6 transition-all hover:border-red-400/30">
          <div className="flex items-center mb-4">
            <h2 className="text-xl md:text-2xl font-bold text-red-500">Invoice Assistant</h2>
            {file && (
              <span className="ml-3 text-xs bg-red-500/20 text-red-300 px-2 py-1 rounded-full">
                {file.name}
              </span>
            )}
          </div>
          ...
          <div className="h-72 md:h-80 overflow-y-auto border border-zinc-700 rounded-lg p-4 bg-zinc-900/50 shadow-inner mb-4">
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-gray-500">
                ...
                <p>No messages yet. Upload an invoice and ask me questions!</p>
              </div>
            ) : (
              messages.map((msg, i) => (
                <div
          ...
          <div className="flex">
            <input
              ...
              placeholder="Ask something about the invoice..."
          ...
          {!file && (
            <p className="text-sm text-red-400 mt-2">Please upload an invoice first to start chatting</p>
          )}
        </section>
      )}
    </main>
